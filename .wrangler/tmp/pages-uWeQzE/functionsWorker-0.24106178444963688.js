...
async function onRequest(context2) {
  ...
  const apiKey = env2.BETTER_AUTH_API_KEY || "ba_hed5bolcn4cj17vepv1wjd1d8nk04pif";
...
async function onRequestPost(context2) {
  ...
  try {
    ...
    const apiKey = env2.GOOGLE_MAPS_API_KEY || "AIzaSyBZ2IVRkU5tGuZFnKqDdIpQmom18AT3AC4";
    ...
    if (data.status === "OK" && data.rows?.[0]?.elements?.[0]?.status === "OK") {
      ...
      const distanceKm = element.distance.value / 1e3;
...
async function onRequestPost2(context2) {
  ...
  try {
    ...
    const amountInCents = Math.round((amount || 15) * 100);
    const stripeKey = env2.STRIPE_SECRET_KEY || env2.STRIPE_SECRET_KEY_TEST;
    if (!stripeKey) {
...
// ../.wrangler/tmp/pages-uWeQzE/functionsRoutes-0.29404328987170114.mjs
var routes = [
  {
